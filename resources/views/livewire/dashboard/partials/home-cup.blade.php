    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        @endif
            ...
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-bold uppercase tracking-widest text-bolao-muted">
                    {{ $match->group_name }}
                </span>
            ...
            <div class="flex items-center justify-between gap-2">
                <div class="flex flex-col items-center gap-2 flex-1">
                    ...
                    <span class="text-xs font-semibold text-slate-200 text-center leading-tight">
                        {{ $match->homeTeam?->short_name ?? $match->homeTeam?->name ?? 'A definir' }}
                    </span>
                </div>
                <div class="flex flex-col items-center min-w-[56px]">
                    <div class="font-bc font-extrabold text-3xl text-white leading-none tracking-widest">
                        {{ $match->home_score_full_time ?? '?' }}–{{ $match->away_score_full_time ?? '?' }}
                    </div>
                    <span class="text-[10px] text-bolao-muted mt-1 uppercase tracking-wide">placar</span>
                </div>
                <div class="flex flex-col items-center gap-2 flex-1">
...
{{-- ── Hero: próximo jogo ────────────────────────────────────── --}}
@if($heroMatch && $live->doesntContain('id', $heroMatch->id))
@php
    $isHeroLive    = $live->contains('id', $heroMatch->id);
...
<section class="px-4 md:px-6 pt-5">
    <div class="flex items-center justify-between mb-3">
        <h2 class="font-bc font-bold text-base uppercase tracking-wide text-white">
            {{ $live->isEmpty() ? 'Próximo Jogo' : 'Em Destaque' }}
        </h2>
        ...
        <div class="border-t border-white/[0.07] px-4 py-3">
            ...
            <div x-data="{
                    ...
                    save() {
                        ...
                        $wire.saveHeroPrediction({{ $heroMatch->id }}, this.h, this.a)
                            .then(r => {
                                this.saving = false;
                                if (r.ok) { this.saved = true; }
                                else { this.errorMsg = r.msg || 'Erro ao salvar.'; }
                            });
                    }
                 }"
...
{{-- ── Main grid: results + standings ──────────────────────── --}}
<div class="grid gap-5 px-4 md:px-6 pt-5 pb-6 lg:grid-cols-2">

    {{-- ── Últimos resultados ─── --}}
    @if($recent->isNotEmpty())
    <div>
        <h2 class="font-bc font-bold text-base uppercase tracking-wide text-white mb-3">
            Últimos Resultados
        </h2>
            ...
            @php
                ...
                if ($pred && $selectedPool) {
                    if ($pred->calculated_at !== null) {
                        ...
                        if ($pts >= $exactPts) {
                            $chipClass = 'pts-exact'; $chipLabel = "+{$pts}";
                        } elseif ($pts > 0) {
                            $chipClass = 'pts-winner'; $chipLabel = "+{$pts}";
                        } else {
                            $chipClass = 'pts-miss'; $chipLabel = '0';
                        }
                    } else {
    ...
    <div>
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[10px] text-bolao-muted uppercase tracking-widest">
                {{ $standingsCriteriaLabel }}
            </span>
        </div>
        <div class="grid grid-cols-2 gap-2 max-h-[420px] overflow-y-auto pr-1">
            @foreach($groupStandings as $standing)
            @php
                $rows = $standing->rows instanceof \Illuminate\Support\Collection
                    ? $standing->rows : collect($standing->rows ?? []);
            @endphp
            <div class="bg-bolao-bg2 border border-white/[0.07] rounded-xl overflow-hidden">
                <div class="flex items-center justify-between px-3 py-2 bg-bolao-bg3 border-b border-white/[0.07]">
                    <span class="font-bc font-bold text-xs uppercase tracking-wider text-slate-300">
                        {{ $standing->group_name }}
                    </span>
                    <span class="text-[9px] text-bolao-green font-semibold">1º e 2º ↑</span>
                </div>
                @foreach($rows->take(4) as $i => $row)
                <div class="flex items-center gap-2 px-3 py-1.5 border-b border-white/[0.04] last:border-0
                            {{ $i < 2 ? 'bg-bolao-green/[0.04]' : '' }}">
                    <span class="text-[11px] font-bold font-bc w-3 text-center
                                 {{ $i < 2 ? 'text-bolao-green' : 'text-bolao-muted2' }}">
                        {{ $i + 1 }}
                    </span>
                    <x-match-team-logo :team="$row->team" size="xs" />
                    <span class="flex-1 text-[11px] font-medium text-slate-200 truncate">
                        {{ $row->team?->tla ?? $row->team?->short_name ?? '—' }}
                    </span>
                    <span class="font-bc font-bold text-sm {{ $i < 2 ? 'text-white' : 'text-bolao-muted' }}">
                        {{ $row->points }}
                    </span>
                </div>
                @endforeach
        ...
        <p class="mt-2 flex items-center gap-1.5 text-[10px] text-bolao-muted">
            <span class="w-2 h-2 rounded-sm bg-bolao-green/30 inline-block"></span>
...
<section class="px-4 md:px-6 pb-4">
    <h2 class="font-bc font-bold text-base uppercase tracking-wide text-white mb-3">Próximos Jogos</h2>
    <div class="flex flex-col gap-1.5">
        @foreach($upcoming->skip(1)->take(5) as $match)
        @php
            $href    = $matchLinks[$match->id] ?? null;
            ...
            $label   = $kickoff
                ...
                : '';
        @endphp
        @if($href)<a href="{{ $href }}" class="bg-bolao-bg2 border border-white/[0.07] rounded-lg px-3 py-2.5 flex items-center gap-3 hover:border-white/[0.13] transition-colors">
        @else<div class="bg-bolao-bg2 border border-white/[0.07] rounded-lg px-3 py-2.5 flex items-center gap-3">
        @endif
            <div class="flex items-center gap-2 flex-1 min-w-0">
                <x-match-team-logo :team="$match->homeTeam" size="sm" />
                <span class="text-sm font-medium text-slate-200 truncate">
...
<div class="rp-widget">
    ...
    <div class="rp-widget-body divide-y divide-white/[0.04]">
        @foreach($upcoming->take(3) as $match)
        @php
            $kickoff = $match->utc_date?->timezone('America/Sao_Paulo');
            $isToday = $kickoff?->isToday();
            $label   = $kickoff
