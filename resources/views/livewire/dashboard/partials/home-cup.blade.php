...
<section class="px-4 md:px-6 pt-5">
    ...
    <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
        ...
        <div class="bg-bolao-bg2 border border-white/[0.07] rounded-xl px-4 py-4 min-h-[138px] relative overflow-hidden opacity-90">
            <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-bolao-accent to-bolao-accent2"></div>
            <div class="flex items-center justify-between mb-3">
                <span class="text-[10px] font-bold uppercase tracking-widest text-bolao-muted">Grupo C</span>
                <span class="inline-flex items-center gap-1 text-[10px] font-bold text-bolao-red">
                    <span class="live-dot mr-0.5"></span> Ao Vivo · 54'
                </span>
            </div>
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-2 flex-1 min-w-0">
                    <x-match-team-logo :team="$simHome" size="sm" />
                    <span class="text-xs font-semibold text-slate-200 truncate">{{ $simHome?->short_name ?? 'BRA' }}</span>
                </div>
                <div class="font-bc font-extrabold text-2xl text-white leading-none tracking-widest shrink-0 min-w-[72px] text-center px-1">1–0</div>
                <div class="flex items-center gap-2 flex-1 min-w-0 justify-end">
                    <span class="text-xs font-semibold text-slate-200 truncate text-right">{{ $simAway?->short_name ?? 'ARG' }}</span>
